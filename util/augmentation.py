...
def magnitude_warp(x, sigma=0.3, knot=10):
    from scipy.interpolate import CubicSpline
    orig_steps = np.arange(x.shape[2])
    
    random_warps = np.random.normal(loc=1.0, scale=sigma, size=(x.shape[0], x.shape[1], knot+2))
...
def window_slice(x, reduce_ratio=0.9):
    ...
    for i, pat in enumerate(x):
        for dim in range(x.shape[1]):
            ret[i,dim,:] = np.interp(np.linspace(0, target_len, num=x.shape[2]), np.arange(target_len), pat[dim, starts[i]:ends[i]]).T
    return ret
...
def window_warp(x, window_ratio=0.1, scales=None):
    ...
    for i, pat in enumerate(x):
        for dim in range(x.shape[1]):
            ...
            ret[i,dim, :] = np.interp(np.arange(x.shape[2]), np.linspace(0, x.shape[2]-1., num=warped.size), warped).T
    return ret
